...
class WarnCommand : CommandExecutor {
    override fun onCommand(
        ...
        if (args.size < 2) {
            senderP.sendMessage(LTCore.PREFIX + langManager.getString(senderP, "command.usage", mapOf("%usage" to "/warn <player> <reason>")))
            return false
        }
        ...
        PunishmentSystem.warnPlayer(senderP, targetName, reasonType, reason)
        return true
    }
}