...
class TpDenyCommand : CommandExecutor {
    override fun onCommand(
        ...
        if (args.isEmpty()) {
            sender.sendMessage(LTCore.PREFIX + langManager.getString(sender as Player, "command.usage", mapOf("%usage" to "/tpdeny <player>")))
            return false
        }
        TpaSystem.denyTpa(sender as Player, args[0])
        return true
    }
}