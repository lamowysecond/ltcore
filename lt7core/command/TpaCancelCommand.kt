...
class TpaCancelCommand : CommandExecutor {
    override fun onCommand(
        ...
        if (args.isEmpty()) {
            sender.sendMessage(LTCore.PREFIX + langManager.getString(sender as Player, "command.usage", mapOf("%usage" to "/tpacancel <player>")))
            return false
        }
        TpaSystem.cancelTpa(sender as Player, args[0])
        return true
    }
}