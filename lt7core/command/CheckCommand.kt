...
class CheckCommand : CommandExecutor {
    override fun onCommand(
        ...
        if (args.isEmpty() || Bukkit.getPlayer(args[0]) == null) {
            sender.sendMessage(LTCore.PREFIX + langManager.getString(sender as Player, "command.usage", mapOf("%usage" to "/check <player>")))
            return false
        }
        CheckingSystem.checkPlayer(sender as Player, Bukkit.getPlayer(args[0])!!)
        return true
    }
}