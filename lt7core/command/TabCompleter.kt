...
class TabCompleter : TabCompleter {
    override fun onTabComplete(
        ...
        return when (command.name.lowercase()) {
            "home", "delhome" -> {
                if (args.size == 1) {
                    HomesSystem.homes.filter { it.key == sender.name }.values.firstOrNull()?.map { it.name } ?: emptyList()
                } else emptyList()
            }

            "addmoney", "removemoney", "resetmoney" -> {
                if (args.size == 1 && sender.hasPermission("lt7core.economy.editmoney")) {
                    MoneySystem.balances.keys.toList()
                } else emptyList()
            }
            ...
            "baltop" -> {
                if (args.size == 1) {
                    (1..(MoneySystem.balances.size / 10)).map { it.toString() }
                } else emptyList()
            }

            "ban", "warn", "banip" -> {
                if (!sender.hasPermission("lt7core.admin.${command.name.replace("temp", "")}")) return emptyList()
                when (args.size) {
                    1 -> Bukkit.getOnlinePlayers().map { it.name }
                    2 -> PunishmentReasonType.entries.map { it.name.replace("_", " ") }
                    else -> emptyList()
                }
            }
            ...
            "mute", "tempban", "tempbanip" -> {
                ...
                when (args.size) {
                    1 -> Bukkit.getOnlinePlayers().map { it.name }
                    2 -> listOf("1m", "5m", "10m", "30m", "1h", "3h", "6h", "12h", "1d", "3d", "7d", "14d", "30d")
                    3 -> PunishmentReasonType.entries.filter {it != PunishmentReasonType.OTHER }.map { it.name.replace("_", " ") }
                    else -> emptyList()
                }
            }
            ...
            "chat" -> {
                if (args.size == 1) listOf("clear", "toggle") else emptyList()
            }
            ...
            "event" -> {
                if (args.size == 1) listOf("start", "end", "join") else emptyList()
            }

            "friend" -> {
                if (args.size == 1) listOf("add", "remove", "cancel", "list") else null
            }

            "lotto" -> {
                if (args.size == 1) listOf("draw", "leave", "info") else emptyList()
            }
