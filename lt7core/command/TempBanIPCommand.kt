...
class TempBanIPCommand : CommandExecutor {
    override fun onCommand(
        ...
        if (args.size < 3) {
            senderP.sendMessage(LTCore.PREFIX + langManager.getString(senderP, "command.usage", mapOf("%usage" to "/tempbanip <player> <time> <reason>")))
            return false
        }
        ...
        PunishmentSystem.banIP(senderP, targetName, reasonType, reason, time)
        return true
    }
}