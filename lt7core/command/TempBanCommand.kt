...
class TempBanCommand : CommandExecutor {
    override fun onCommand(
        ...
        if (args.size < 3) {
            senderP.sendMessage(LTCore.PREFIX + langManager.getString(senderP, "command.usage", mapOf("%usage" to "/tempban <player> <time> <reason>")))
            return false
        }
        ...
        PunishmentSystem.banPlayer(senderP, targetName, reasonType, reason, time)
        return true
    }
}