...
object ChangesSystem {
    ...
    fun sendChangeRequest(sender: Player, target: String, money: Double) {
        ...
        println(player.openInventory.topInventory.type.name)
        ...
        sender.inventory.removeItem(itemInHand)
        ...
        Bukkit.getScheduler().runTaskLater(LTCore.instance, Runnable {
            if (activeChanges.contains(changeRequest)) {
                activeChanges.remove(changeRequest)
                sender.sendMessage(LTCore.PREFIX + langManager.getString(sender, "changes.request_expired", mapOf("%p" to target)))
                player.sendMessage(LTCore.PREFIX + langManager.getString(player, "changes.request_expired_target", mapOf("%a" to sender.name)))
                sender.addItemOrDrop(itemInHand)
            }
        }, config.friendsRequestTime * 20L)

        sender.sendMessage(LTCore.PREFIX + langManager.getString(sender, "changes.request_sent", mapOf("%a" to target)))
                ...
                listOf(
                    ...
                    GuiButton(
                        ...
                        ),
                        {
                            ...
                            player.sendMessage(LTCore.PREFIX + langManager.getString(player, "changes.request_accepted_target", mapOf("%p" to sender.name)))

                            MoneySystem.removeMoney(player.name, money)
                            MoneySystem.addMoney(sender.name, money)
                    ...
                    GuiButton(
                        ...
                        {
                            activeChanges.removeIf { it.player.name == sender.name && it.target.name == player.name }
                            sender.sendMessage(LTCore.PREFIX + langManager.getString(sender, "changes.request_declined", mapOf("%a" to player.name)))
