...
object ShopSystem {
    ...
    val shopItems: MutableList<ShopCategory> = mutableListOf()
    ...
    private fun openCart(
        ...
        Gui(
            ...
            buttons = carts.getOrDefault(player, mutableListOf()).map {
                ...
                GuiButton(
                    slot = x + 10 + 3 * (x / 7),
                    icon = itemStack(it.shopItem.item.type, lore = listOf(
                    ...
                    onClick = { event ->
                        if (event.click.isLeftClick) {
                            if (it.quantity < 0) {
                                ...
                                if (absQuantity > player.countExactItem(it.shopItem.item)) {
    ...
    private fun getCategoryPage(
        ...
        Gui(
            ...
            buttons = category.items.map {
                ...
                GuiButton(
                    slot = x + 10 + 3 * (x / 7),
                    icon = itemStack(it.item.type, lore = listOf(
    ...
    private fun sellItem(
        ...
        val quantityInInventory = player.countExactItem(item.item)
        ...
        for (invItem in player.inventory.contents) {
            if (invItem != null &&
                invItem.type == item.item.type &&
                invItem.itemMeta?.displayName == item.item.itemMeta?.displayName &&
                invItem.itemMeta?.lore == item.item.itemMeta?.lore
            ) {
        ...
        player.sendMessage(LTCore.PREFIX + langManager.getString(player, "shop.message.sellsuccess", mapOf("%i" to quantity.toString(), "%m" to moneyEarned.toString())))
    }
    ...
    private fun buyItem(
        ...
        player.addItemOrDrop(item.item.clone().apply { this.amount = quantity })
        player.sendMessage(LTCore.PREFIX + langManager.getString(player, "shop.message.buysuccess", mapOf("%i" to quantity.toString(), "%m" to totalPrice.toString())))
    }
    ...
    private fun getPaymentPage(
        ...
        Gui(
            ...
            buttons = mutableListOf(
                GuiButton(
                    slot = 13,
                    icon = itemStack(item.item.type, lore = listOf(
                ...
                if (player.hasExactItem(item.item)) {
                    add(
                        GuiButton(
                            slot = 52,
                            icon = itemStack(Material.LIME_WOOL, name = langManager.getString(player, "shop.gui.payment.sellall")),
                            onClick = { _ ->
                                sellItem(player, item, player.countExactItem(item.item))
                            }
                        )
                    )
                }
                ...
                if (quantity < 0) {
                    ...
                    if (absQuantity > player.countExactItem(item.item)) return@apply
                } else {
