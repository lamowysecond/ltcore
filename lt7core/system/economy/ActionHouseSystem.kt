...
object ActionHouseSystem {
    ...
    val items = mutableListOf<ActionHouseItem>()
                ...
                GuiButton(
                    ...
                    onClick = {
                        if (sender.name == ahItem.seller) {
                            ...
                            Bukkit.getPlayer(ahItem.seller)?.sendMessage(LTCore.PREFIX + langManager.getString(sender, "action-house.gui.item.sold_notification", mapOf("%p" to ahItem.price.toString(), "%b" to MoneySystem.getBalance(ahItem.seller).toString())))
                        }
                    }
                )
            }.toMutableList().apply {
