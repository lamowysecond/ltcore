...
object TpaSystem {
    ...
    fun sendTpa(player: Player, target: String) {
        ...
        if (!IgnoresSystem.isIgnoring(target, player.name)) {
            targetPlayer.sendMessage(LTCore.PREFIX + langManager.getString(targetPlayer, "tpa.request_received", mapOf("%p" to player.name)))
            targetPlayer.sendMessage(
                Component.text(langManager.getString(targetPlayer, "tpa.request_accept_btn"))
    ...
    fun denyTpa(player: Player, senderName: String) {
        ...
        Bukkit.getPlayer(senderName)?.sendMessage(LTCore.PREFIX + langManager.getString(Bukkit.getPlayer(senderName)!!, "tpa.request_denied_by", mapOf("%p" to player.name)))
    }
