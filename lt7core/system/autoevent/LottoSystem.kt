...
object LottoSystem {
    ...
    init {
        Bukkit.getScheduler().runTaskTimer(LTCore.instance, Runnable {
            ...
            val totalPrize = lottoData.size * config.lottoBet.toDouble() + if (lastLotto?.winners?.isEmpty() == true) lastLotto?.totalPrize!! else 0.0
            lastLotto = Lotto(
                winners,
                winningNumber,
                totalPrize
            ...
            Utils.broadcast("lotto.draw_announcement",
                mapOf(
                    ...
                    "%ap" to if (prizePerWinner == 0.0) "-" else prizePerWinner.toString()
                )
            )
        }, 0L, 60 * 20L)
    }
    ...
    fun viewLottoInfo(sender: Player) {
        ...
        sender.sendMessage(LTCore.PREFIX + LTCore.langManager.getString(sender, "lotto.total_bet",
            mapOf(
                "%a" to (config.lottoBet * entries.size + if (lastLotto?.winners?.isEmpty() == true) lastLotto?.totalPrize!! else 0.0).toString()
            )
        ))
